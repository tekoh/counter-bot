...
async function getMember(message, memberName) {
  ...
  for (let member of members.keys()) {
    ...
    if (member.user.id == memberName) {
      ...
      member.user.tag.toLowerCase().includes(memberName.toLowerCase())
    ) {
      if (member.user.bot) {
      ...
      member.displayName.toLowerCase().includes(memberName.toLowerCase())
    ) {
      if (member.user.bot) {
...
function addUse(quote, memberId) {
  db.prepare("UPDATE quotes SET mentions = mentions + 1 WHERE quote = ?").run(
    quote
  );
  db.prepare("INSERT INTO history (user, quote, date) VALUES (?, ?, ?)")
    .run(memberId, quote, Date.now())
    .run();
}
...
function getMentions(quote) {
  const query = db
    .prepare("SELECT mentions FROM quotes WHERE quote = ?")
    .get(quote);
